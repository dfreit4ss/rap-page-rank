...
(defn insert-in-graph
  ...
  (if (= (first vector-of-matches) artist-name)
    ...
    (if (= 0 (second (first vector-of-matches)))
      (insert-in-graph (rest vector-of-matches) artist-name) ;; if appears 0 times i don't need put it on graph
      (do
        (uber/add-directed-edges page-rank-graph [artist-name (first (first vector-of-matches)) (second (first vector-of-matches))])
...
;; define one graph then use add-edges using the vector generated by find-artists
